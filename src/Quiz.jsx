...
// Honest evidence-strength labels per compound. Deliberately conservative —
// "Emerging"/"Limited" where the human data is thin, so the tool never oversells.
const EVIDENCE = {
  ...
  tyrosine: { level: "Moderate", note: "Best-supported under acute stress, sleep loss, or heavy cognitive load — not everyday baseline." },
...
// Maps a "current stack" quiz answer to the compound it covers, so we can tell
// people which recommended compounds they're already taking instead of silently
// dropping them.
...
// Timeline buckets, ordered soonest → slowest.
const ONSET_BUCKETS = [
...
function buildStack(answers) {
  ...
  // Of the compounds we'd recommend for this goal, which is the user already taking?
  ...
  // Build the "what kicks in when" timeline from the selected compounds.
...
// Packs the structured quiz answers into a compact, URL-safe token so a
// protocol can be reopened on refresh or shared via a link — no backend needed.
// The free-text "notes" field is intentionally left OUT of the link to keep
// anything personal someone typed from ending up in a shared URL.
...
function decodeAnswers(code) {
  try {
    ...
    const answers = {
      ...
      budget: p.b,
      commitment: p.k,
      current: Array.isArray(p.c) ? p.c : [],
      restrictions: Array.isArray(p.r) ? p.r : [],
      medications: Array.isArray(p.m) ? p.m : [],
    };
    // A goal is required to rebuild a stack — bail out cleanly if the link is junk.
...
.ng-share-btn {
  ...
  cursor: pointer;
  background: linear-gradient(135deg, var(--forest) 0%, var(--forest2) 100%);
  color: white; border: 1px solid var(--forest);
  box-shadow: 0 4px 18px rgba(13,107,63,0.22);
...
.ng-share-btn-alt {
  ...
  border: 1px solid var(--bg2); box-shadow: var(--shadow-sm);
}
...
.ng-share-hint {
  ...
  font-weight: 300; margin: -16px 0 28px;
}

/* ── Print: turn the results screen into a clean printable / save-as-PDF sheet ── */
/* ── Affiliate note under stack heading ── */
.ng-affil-note {
  font-size: 12px; color: var(--ink4); font-weight: 300;
...
.ng-ev-badge {
  ...
  padding: 4px 10px; border-radius: 40px; white-space: nowrap;
}
...
.ng-already-title {
  font-family: 'Azeret Mono', monospace;
  font-size: 11px; letter-spacing: 2px; text-transform: uppercase;
  color: var(--forest); margin-bottom: 8px; font-weight: 600;
}
...
.ng-tl-marker {
  display: flex; flex-direction: column; align-items: center; flex-shrink: 0; padding-top: 5px;
}

.ng-tl-dot {
  width: 12px; height: 12px; border-radius: 50%;
  background: var(--forest); box-shadow: 0 0 0 4px rgba(13,107,63,0.12);
}
...
.ng-tl-when {
  font-family: 'Azeret Mono', monospace;
  font-size: 11px; letter-spacing: 1px; text-transform: uppercase; color: var(--forest); font-weight: 600;
}
...
/* ── Accessibility: quiz option + control focus ── */
.ng-opt {
  font-family: inherit;
  text-align: left;
  width: 100%;
...
.ng-opt:focus-visible,
.ng-share-btn:focus-visible,
.ng-start-btn:focus-visible,
.ng-btn:focus-visible,
.ng-ft-tag:focus-visible,
.ng-restart:focus-visible {
  outline: 2px solid var(--forest);
  outline-offset: 2px;
}

@media print {
  .ng-bg, .ng-grain, .ng-share-bar, .ng-share-hint,
  .ng-cta-box, .ng-restart, .ng-quiz-top { display: none !important; }
  body { background: #fff !important; }
  .ng-app, .ng-content, .ng-results { background: #fff !important; }
  .ng-results { padding-top: 12px !important; max-width: 100% !important; }
  .ng-supp, .ng-notes-card, .ng-schedule, .ng-med-section, .ng-warn {
...
export default function NootraGenie() {
  ...
  // If the page is opened at /r/:code (a shared or bookmarked link), decode the
  // answers from the URL and jump straight to the rebuilt protocol.
  useEffect(() => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: "My NootraGenie Protocol",
          text: "Here's the personalized nootropic stack NootraGenie built for me:",
          url,
        });
        return;
      } catch (e) {
        ...
        <div className="ng-content">
          ...
          {screen === "welcome" && (
            <div className="ng-welcome">
              ...
              <div style={{ marginTop: '12px', display: 'flex', gap: '18px', justifyContent: 'center' }}>
              ...
              <div className="ng-quiz-body">
                ...
                {q.freetext ? (
                  ...
                  <div className="ng-options">
                    {q.options.map((o) => (
                      <button type="button" key={o.value} className={`ng-opt${isActive(o.value) ? " active" : ""}`} aria-pressed={isActive(o.value)} onClick={() => select(o.value)}>
            ...
            <div className="ng-results ng-fade">
              {error ? (
                ...
                <>
                  ...
                  <p className="ng-affil-note">Heads up: the "Buy on Amazon" links below are affiliate links — we may earn a small commission at no extra cost to you.</p>
                  ...
                  {results.alreadyHave && results.alreadyHave.length > 0 && (
                    <div className="ng-already">
                      ...
                      <p>
                        You're already taking {results.alreadyHave.length === 1 ? results.alreadyHave[0] : results.alreadyHave.slice(0, -1).join(", ") + " and " + results.alreadyHave.slice(-1)} — {results.alreadyHave.length === 1 ? "it's" : "they're"} part of what we'd recommend for your goal, so we left {results.alreadyHave.length === 1 ? "it" : "them"} off the list above to avoid doubling up. You're covered there; keep it going.
                      </p>
                    </div>
                  )}
